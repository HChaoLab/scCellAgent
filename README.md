...
6. **产物结构固定化**：输出目录和报告模板标准化，方便复用和审计。
...
3. Agent 根据重点关注内容生成分析计划与生物学假设。
4. Agent 逐步请求模型产出小块代码。
5. 本地执行 `validator` 拦截幻觉变量、危险导入、未注册 API。
6. 用受控 Python 子进程执行代码，并把执行结果写入本地状态。
7. 若产生关键图，调用 MiniMax 视觉模型做图形质量评审。
8. 技术评价和生物学评价分别给出结论。
9. 在设定次数内进行技术迭代或假设迭代。
10. 生成标准化输出目录与多类报告。
...
2. 把 `scanpy/anndata/pandas/numpy/matplotlib` 注册为受控工具。
3. 对关键图统一导出 `.png + .pdf`。
4. 为 QC、聚类、差异表达、细胞注释分别设计独立 prompt 模板。
5. 将 `state.json` 与输出目录纳入实验审计链路。
